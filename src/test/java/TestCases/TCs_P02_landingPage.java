...
public class TCs_P02_landingPage extends BaseTests {
    ...
    @Description("Reset the app state and validate cart items are cleared.")
    @Story("User Story #21 - As a user, " +
            "I want to reset the app to clear the cart and restore item states.")
    public void resetAppItem() throws IOException {
        log.info("Starting: resetAppItem");
        ...
        loginPage.enterUserName(Constants.loginUsername)
                ...
                .click_On_Menu()
                .click_On_RestAppItem();

        String cartCount = selectProduct.getNumberOfProductsOnCart();
        log.info("Number of products on cart after reset: {}", cartCount);
        Assert.assertEquals(cartCount, "0", "Cart should be empty after reset");

        String buttonStatus = driver.findElement(By.xpath("//button[@class]")).getText();
        log.info("Button text after reset: {}", buttonStatus);

        ScreenShot.takeFullScreenShot(driver, "ResetAppItem_Verification");
        Assert.assertEquals(buttonStatus, "Remove", "Button did not reset correctly");
    }
    ...
    public void clickOnCartIconTC() {
        ...
        Assert.assertEquals(actualURL, Constants.loginBaseURL, "User is not redirected to cart page");
    }
